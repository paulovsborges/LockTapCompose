package com.pvsb.locktapcompose.presentation.main.categories.sessionOptions
...
enum class SessionOptionsButton(val id: Int) {
    PRIVATE_CONTACTS(0), WIFI_PASSWORDS(1), PHOTO_VAULT(2),
}
        ...
        Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
            repeat(optionsLabels.size) { btnId ->
                ComposeSessionOptionButton(label = optionsLabels[btnId], onClick = {
                    onOptionClicked(btnId, navController)
                })
            }
        }
    }
...
@Preview
@Composable
private fun SessionOptionsScreenPreview() {
    val navController = rememberNavController()
    SessionOptionsScreen(navController, MainScreens.Categories.label)
}